...
def cutwire(bot, trigger):
    ...
    if wirecut.lower() in ('all', 'all!'):
        ...
        if bot.privileges[trigger.sender][bot.nick] >= OP:
            ...
            bot.reply('No! No, that\'s the wrong one. Aww, you\'ve gone and killed yourself. Oh, that\'s... that\'s not good. No good at all, really. Wow. Sorry. (You should\'ve picked the ' + color + ' wire.)')


def explode(bot, trigger):
    global bombs, colors
    target = trigger.group().split()[1]
    kmsg = 'KICK ' + trigger.sender + ' ' + target + \
    ...
    if bot.privileges[trigger.sender][bot.nick] >= OP:
        ...
        bot.reply('Oh, come on, ' + target + '! You could\'ve at least picked one! Now you\'re dead. Guts, all over the place. You see that? Guts, all over YourPants. (You should\'ve picked the ' + bombs[target.lower()][0] + ' wire.)')
